...
export const validateProfile = (data: User | UserRegister) => {
    ...
    if (!nameRegex.test(data.name)) {
        err = "Por favor, nome sem números nem caracteres especiais.";
      return err;
    }
    ...
    if (!nifRegex.test(data.nif)) {
        err = "Por favor, nif coloque como está no seu Bilhete de Identificação.";
      return err;
    }
  ...
  export const validateRegister = (data: UserRegister) => {
      let err: string | null = null;
    if (!data.name.trim() || !data.nif.trim() || !data.email.trim()) {
        err = "Por favor, preencha todos os campos obrigatórios.";
      return err;
    ...
    if (data.password.length < 6) {
      err = "A palavra-passe deve ter pelo menos 6 caracteres.";
      return err;
    }
    err = null;
    return err
